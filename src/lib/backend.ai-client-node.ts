...
class Client {
  ...
  _updateSupportList() {
    ...
    if (this.isManagerVersionCompatibleWith('26.8.0')) {
      ...
      // a 26.7.0 manager an omitted `shell` is null, which disables shell
      // wrapping and turns the Basic mode into Exec. Worse, a manager built
      // from `main` after BA-6742 still reports `26.7.0`, so the two cannot be
      // told apart by version. BA-6742 ships together with the 26.8.0 fields
      // (both are in `main`), so gating the whole command/shell path at 26.8.0
      // removes the ambiguity — 26.7.0 managers keep using the deprecated
      // `startCommand` token list and never receive `shell`. Kept in sync
      // with the browser client (`packages/backend.ai-client/src/client.ts`).
